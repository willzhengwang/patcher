...
def test_1d():
    ...
    mosaic = unpatch(patches, out_shape=arr.shape, step=3)
    ...
    arr = np.arange(9)
    patches = patch(arr, (5, ), step=3, do_pad=False)
...
def test_2d():
    ...
    patches = patch(arr, patch_shape, step=step, do_pad=False)

    # case 1 - 1: out_shape is None
    mosaic = unpatch(patches, step=step)
    assert np.allclose(arr[:5, :8], mosaic)

    # case 1 - 2: out_shape is larger than the originally merged array shape
    mosaic = unpatch(patches, out_shape=arr.shape, step=step)
...
def test_4d():
    ...
    patches = patch(arr, (5, 5, 3, 2), step=(3, 3, 2, 2), do_pad=True)
    assert patches.shape == (2, 3, 3, 1, 5, 5, 3, 2)
    mosaic = unpatch(patches, out_shape=arr.shape, step=(3, 3, 2, 2))
    assert np.allclose(arr, mosaic)
